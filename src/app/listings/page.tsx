...
export default function ListingsPage() {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      // Fetch both types - looking for roommate and spare rooms
      const [roommateRes, spareRoomRes] = await Promise.all([
        postsApi.getPosts({
      ...
      const allPosts = [...roommateRes.data, ...spareRoomRes.data].sort(
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-neutral-900 dark:text-neutral-100 flex items-center gap-2">
                <Home className="w-5 h-5 text-[var(--teal-500)]" />
                Available Listings
              </h1>
              <p className="text-xs text-neutral-500">
                {posts.length} listing{posts.length !== 1 ? 's' : ''} available
              </p>
            </div>
          </div>
            ...
            <button
              ...
              className={cn(
                ...
                (location || budgetRange !== 'any')
                  ? 'bg-[var(--teal-100)] dark:bg-[var(--teal-500)]/20 text-[var(--teal-600)]'
      ...
      <main className="max-w-lg mx-auto px-4 py-6">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <Loader2 className="w-10 h-10 text-[var(--teal-500)] animate-spin mb-4" />
            <p className="text-neutral-500">Finding listings...</p>
          </div>
          ...
          <div className="text-center py-20">
            <div className="w-20 h-20 rounded-full bg-[var(--peach-100)] dark:bg-neutral-800 flex items-center justify-center mx-auto mb-4">
              <Home className="w-10 h-10 text-[var(--peach-400)]" />
            </div>
            <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100 mb-2">
              {showEmptyState ? "You've seen them all!" : 'No listings found'}
            </h2>
            ...
            <button
              ...
              className="px-6 py-3 rounded-full bg-gradient-to-r from-[var(--teal-400)] to-[var(--lime-400)] text-white font-semibold flex items-center gap-2 mx-auto hover:opacity-90 transition-opacity"
            >
            ...
            >
              <div className="sticky top-0 bg-white dark:bg-neutral-900 px-5 py-4 border-b border-neutral-200 dark:border-neutral-800">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-sm text-[var(--teal-500)] font-medium"
                  >
                    Clear All
                  </button>
                </div>
              ...
              <div className="sticky bottom-0 bg-white dark:bg-neutral-900 p-5 border-t border-neutral-200 dark:border-neutral-800">
                <button
                  ...
                  className="w-full py-4 rounded-xl bg-gradient-to-r from-[var(--teal-400)] to-[var(--lime-400)] text-white font-semibold hover:opacity-90 transition-opacity"
                >
                  Apply Filters
                </button>
              </div>
